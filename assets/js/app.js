...
function implementorview(){
    $('.iv').show();
    $('.rv').hide();
    $('#dbtn').html("Assign another implementor");

    $('#abtn').click(function(){
        $('.temp1').fadeOut();
        $('#abtn').fadeOut(300);
        $('#bbtn').delay(300).fadeIn();
        notify("Project has been claimed","success",5);
        $('#bbtn').click(function(){
            $('#bbtn').html("Assign Validator").addClass('bbtn s-ws-top');
            $('.temp2').delay(300).fadeIn();
            $('.bbtn').click(function(e){
                e.preventDefault();
                $('.temp2').fadeOut();
                $('.bbtn').fadeOut();
                $('#assignarea').delay(300).slideDown();
            })
        });
    });

    $('#projects .plistli').click(function(){
        $('#implementorview').fadeOut(300);
        $("#singleprojectview").delay(300).fadeIn();
    });
    $('#idealist .plistli').click(function(){
        $('#implementorview').fadeOut(300);
...
$('#newidea').submit(function(event)
{
    ...
    setTimeout(notify("Your idea has been added successfully","success",5), 10000);
    addidea($('#ideatitle').val(),$('#ideabody').val());
});

function addidea(title, body){
    $('#timelinecards').prepend('<div class="panel" id="card1" style="display:none;"><div class="row"> <div class="small-1 columns"> <i class="icon-idea yc f-2x"></i> </div> <div class="small-9 columns"> <h5>'+title+'<span class="btn tiny"><a class="siv scolor2 xs-ws-left">View Idea <i class="icon-right s2"></i></span></a></h5> </div> <div class="small-2 end columns scolor2 text-right s2"> <i class="icon-clock"></i> 2 sec ago</div> </div> <div class="row"> <div class="small-12 columns s-ws-top">'+body+'</div></div><hr> <div class="row"><div class="small-2 columns"><img src="https://randomuser.me/api/portraits/med/men/96.jpg" class="circle-img"></div> <div class="small-10 columns s-ws-top"> <h6 class="scolor nm">Aditya Nayak</h6><div class="s2 scolor2 xs-ws-top"><i class="icon-location"></i>Account manager</div></div> </div> </div> </div>')
    $('#card1').delay(1000).slideDown(500);
}
var cp;
